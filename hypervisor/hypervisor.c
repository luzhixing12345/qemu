...
VM *kvm_init(uint8_t code[], size_t len) {
    ...
    struct kvm_userspace_memory_region region = {
        .slot = 0, .flags = 0, .guest_phys_addr = 0, .memory_size = MEM_SIZE, .userspace_addr = (size_t)mem};
    ...
    *vm = (struct VM){.mem = mem, .mem_size = MEM_SIZE, .vcpufd = vcpufd, .run = run};
...
#define STACK_ALLOC(sp, len) \
    ...
    *(uint64_t *)STACK_ALLOC(sp, sizeof(char *)) = 0;
    for (int i = argc - 1; i >= 0; i--) *(uint64_t *)STACK_ALLOC(sp, sizeof(char *)) = copy[i] - (char *)vm->mem;
    /* push argc */
