...
void *mmap(void *addr, uint64_t len, int prot) {
    ...
    /* no ASLR */
    if (last_mmapped == (void *)-1)
        last_mmapped = (void *)0x00007ffff7fff000ull;

    if (addr == 0)
        last_mmapped = addr = last_mmapped - len;
    for (uint64_t i = 0; i < len; i += 0x1000) add_trans_user(addr + i, ret + i, prot);
    return addr;
}
