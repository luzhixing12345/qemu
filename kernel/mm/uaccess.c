...
int access_ok(int type, const void *addr_, uint64_t size) {
    ...
    if (!USER_MEM_RANGE_OK(addr))
        return 0;
    if (!USER_MEM_RANGE_OK(addr + size - 1))
        return 0;
    for (uint64_t v = aligndown(addr); v < alignup(addr + size); v += 0x1000)
        if (translate((void *)v, 1, type) == (uint64_t)-1)
            return 0;
    return 1;
}
...
int access_string_ok(const void *addr_) {
    if (!access_ok(VERIFY_READ, addr_, 1))
        return 0;
    uint64_t addr = (uint64_t)addr_;
    ...
    if (l == remain_size)
        return access_string_ok((void *)(addr + l));
    return 1;
}
