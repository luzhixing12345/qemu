...
static int load_binary(int fd, process *p) {
    ...
    if (ehdr->e_type == ET_DYN)
        p->load_addr = 0x0000555555554000ull;  // who care ASLR
    ...
    for (int i = 0; i < ehdr->e_phnum; i++, phdr++)
        if (phdr->p_type == PT_LOAD) {
            ...
            uint64_t st = p->load_addr + aligndown(phdr->p_vaddr), ed = p->load_addr + alignup(phdr->p_vaddr + sz);
            int prot = pf_to_prot(phdr->p_flags);
            /* not a good idea, but it works */
            void *r =
                sys_mmap((void *)st, sz + (phdr->p_offset & 0xfff), prot, MAP_FIXED, fd, phdr->p_offset & -0x1000);
            if (r != (void *)st)
                return (int)(int64_t)r;  // error returned by sys_mmap
...
static int check_and_get_count(char *const ary[]) {
    if (!access_ok(VERIFY_READ, ary, 8))
        return -EFAULT;
    int i = 0;
    while (*ary != 0) {
        if (!access_string_ok(*ary))
            return -EFAULT;
        ++ary;
        ++i;
        if (!access_ok(VERIFY_READ, ary, 8))
...
static int create_elf_info(process *p, char *const argv[], char *const envp[]) {
    ...
    for (int i = argc + envc + 1; i >= 0; i--) *(char **)STACK_ALLOC(p->rsp, 8) = copy[i];
    kfree(copy);
...
int sys_execve(const char *path, char *const argv[], char *const envp[]) {
    ...
    asm volatile(
        ...
        "xor rbp, rbp;"
        ".byte 0x48;"
        "sysretq" ::[entry] "r"(p.entry + p.load_addr),
        [rsp] "r"(p.rsp)
        : "r11", "rcx");
    /* never reached */
    return -EPERM;
